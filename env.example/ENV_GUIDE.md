...
- 默认值：8（自动根据 CPU 核数计算）
- **智能特性（v2.0+）**：
  - **无前台任务时自动加速**：首次索引/重建时，若用户未打开网页（无缩略图任务），自动提升 50%-100% 并发
  - **有前台任务时自动让路**：用户浏览相册时，自动降低索引并发为前台让路
  - **每批次实时评估**：每处理 1000 个文件重新检测前台负载并动态调整
- 取值/格式：1-32的整数（仅作为降级兜底值）
- **推荐配置方案**：
  - **通常情况**：无需配置（系统自动管理）
  - **强制固定并发**：如需禁用动态调整，可手动设置（不推荐）
  - **降级兜底**：adaptive.service 不可用时使用此值
- 动态调整示例（8核服务器）：
  - 首次索引 + 无前台任务 → 12 并发（+50%）
  - 首次索引 + 用户浏览相册 → 4 并发（-50%，为前台让路）
