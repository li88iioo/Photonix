...
/**
 * @function generateImageFromChat
 * @desc AI密语图片生成：从聊天的当前图片生成新图片
 * @param {object} req - Express 请求对象
 * @param {object} res - Express 响应对象
 * @returns {Promise<void>} JSON 响应
 */
exports.generateImageFromChat = async (req, res) => {
    const { image_path, visionConfig, userInstruction } = req.body || {};
    ...
    if (!visionConfig || !visionConfig.url || !visionConfig.key || !visionConfig.model) {
        return res.status(400).json({
            code: 'VISION_CONFIG_INCOMPLETE',
            message: '视觉模型配置不完整',
            requestId: req.requestId
        });
    }
    ...
    try {
        const aiImageService = require('../services/ai-image-generation');

        // 检查服务是否已配置
        if (!aiImageService.isConfigured()) {
            return res.status(503).json({
                code: 'IMAGE_GEN_NOT_CONFIGURED',
                message: '图片生成服务未配置，请联系管理员设置环境变量',
                requestId: req.requestId
            });
        }

        if (userInstruction) {
            logger.info(`[AI-ImageGen] 收到图片生成请求: ${sanitizedPath}（用户指令: ${userInstruction}）`);
        } else {
            logger.info(`[AI-ImageGen] 收到图片生成请求: ${sanitizedPath}`);
        }

        // 执行图片生成（包含用户指令）
        const result = await aiImageService.generateImageFromPhoto(sanitizedPath, visionConfig, userInstruction || '');
 ...
 * @desc 检查AI图片生成功能是否已配置并可用
 * @param {object} req - Express 请求对象
 * @param {object} res - Express 响应对象
 * @returns {Promise<void>} JSON 响应
 */
exports.getImageGenStatus = async (req, res) => {
    try {
        const aiImageService = require('../services/ai-image-generation');
        const isConfigured = aiImageService.isConfigured();

        return res.status(200).json({
            enabled: isConfigured,
            message: isConfigured
                ? '视觉提取文生图功能已启用'
                : '视觉提取文生图功能未配置，需要设置 AI_IMAGE_GEN_URL 和 AI_IMAGE_GEN_KEY',
            requestId: req.requestId
        });
    } catch (error) {
        logger.error(`[AI-ImageGen] 获取图片生成状态失败: ${error.message}`);
