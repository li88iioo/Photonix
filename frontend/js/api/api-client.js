 ...
 * @file api-client.js
 * @description
 *   统一 API 客户端，内置错误处理与重试策略，提供标准化的请求方法与策略预设。
 */

import { NETWORK } from '../core/constants.js';
import { getAuthToken, setAuthToken } from '../app/auth.js';
 ...
 * @param {number} [baseDelay=NETWORK.RETRY_BASE_DELAY] 基础延迟（毫秒）
 ...
 * @param {number} [policy.maxRetries] 最大重试次数
 * @param {number} [policy.timeout] 超时时间（毫秒）
 * @param {number} [policy.retryDelay] 初始重试延迟（毫秒）
 ...
 * @param {object} [options.policy] 重试策略
 ...
 * @param {number} [config.retryDelay] 初始重试延迟（毫秒）
 * @returns {object} 请求策略对象
 */
...
/**
 * 请求策略预设
 * @namespace RequestPolicies
 * @property {object} FAST     快速请求：低重试次数，短超时
 * @property {object} STANDARD 标准请求：平衡的重试和超时
 * @property {object} RELIABLE 可靠请求：高重试次数，长超时
 * @property {object} REALTIME 实时请求：无重试，短超时
 */
