...
import { DropdownController } from '../../shared/dropdown-controller.js';
...
export function renderSortDropdown() {
    ...
    const dropdownOptions = Object.entries(sortOptions).map(([value, label]) => createElement('button', { classes: ['sort-option','w-full','text-left','px-3','py-2','text-sm','text-white','hover:bg-gray-700','transition-colors',...(currentOption === value ? ['bg-purple-600'] : [])], attributes: { 'data-value': value, role: 'menuitemradio', 'aria-checked': String(currentOption === value) }, textContent: label }));
    const sortDropdown = createElement('div', { classes: ['absolute','top-full','right-0','mt-1','bg-gray-800','border','border-gray-700','rounded-lg','shadow-lg','z-50','hidden','w-full'], attributes: { id: 'sort-dropdown', role: 'menu' }, children: dropdownOptions });
    ...
    // Accessible dropdown controller
    const controller = new DropdownController({
        toggleButton: sortButton,
        menu: sortDropdown,
        onOpen: () => safeClassList(iconContainer, 'add', 'rotate-180'),
        onClose: () => safeClassList(iconContainer, 'remove', 'rotate-180')
    });
    ...
    dropdownOptions.forEach(option => {
        option.addEventListener('click', (e) => {
            ...
            dropdownOptions.forEach(opt => {
                safeClassList(opt, 'remove', 'bg-purple-600');
