...
import { applyMasonryLayout, triggerMasonryUpdate, ensureMasonryResizeObserver } from './masonry.js';
import { MATH, UI, isColumnsFallbackEnabled } from '../../core/constants.js';
...
export function displayAlbum(album) {
    ...
    const img = createElement('img', { classes: ['w-full','h-full','object-cover','absolute','inset-0','lazy-image','transition-opacity','duration-300'], attributes: { src: "data:image/svg+xml,%3Csvg xmlns='http://www.w3.org/2000/svg' viewBox='0 0 1 1'%3E%3C/svg%3E", 'data-src': album.coverUrl, alt: album.name, loading: 'lazy' } });
...
export function displayStreamedMedia(type, mediaData, index, showTimestamp) {
    ...
    if (isVideo) {
        kids.push(createElement('img', { classes: ['w-full','h-full','object-cover','absolute','inset-0','lazy-image','transition-opacity','duration-300'], attributes: { src: "data:image/svg+xml,%3Csvg xmlns='http://www.w3.org/2000/svg' viewBox='0 0 1 1'%3E%3C/svg%3E", 'data-src': mediaData.thumbnailUrl, alt: '视频缩略图', loading: 'lazy' } }));
        ...
        kids.push(createElement('img', { classes: ['w-full','h-full','object-cover','absolute','inset-0','lazy-image','transition-opacity','duration-300'], attributes: { src: "data:image/svg+xml,%3Csvg xmlns='http://www.w3.org/2000/svg' viewBox='0 0 1 1'%3E%3C/svg%3E", 'data-src': mediaData.thumbnailUrl, alt: '图片缩略图', loading: 'lazy' } }));
    }
...
export function displaySearchMedia(result, index) {
    ...
    if (isVideo) {
        kids.push(createElement('img', { classes: ['w-full','h-full','object-cover','absolute','inset-0','lazy-image','transition-opacity','duration-300'], attributes: { src: "data:image/svg+xml,%3Csvg xmlns='http://www.w3.org/2000/svg' viewBox='0 0 1 1'%3E%3C/svg%3E", 'data-src': result.thumbnailUrl, alt: `视频缩略图：${result.name}`, loading: 'lazy' } }));
        const overlay = createElement('div', { classes: ['video-thumbnail-overlay'] });
        const playBtn = createElement('div', { classes: ['video-play-button'] });
        const playSvg = createPlayButton();
        playBtn.appendChild(playSvg);
        ...
        kids.push(createElement('img', { classes: ['w-full','h-full','object-cover','absolute','inset-0','lazy-image','transition-opacity','duration-300'], attributes: { src: "data:image/svg+xml,%3Csvg xmlns='http://www.w3.org/2000/svg' viewBox='0 0 1 1'%3E%3C/svg%3E", 'data-src': result.thumbnailUrl, alt: result.name, loading: 'lazy' } }));
    }
    // 为所有媒体添加时间戳（视频和图片）
    if (timeText) kids.push(createElement('div', { classes: ['absolute','bottom-2','right-2','bg-black/50','text-white','text-sm','px-2','py-1','rounded','shadow-lg'], textContent: timeText }));
...
export function applyLayoutMode() {
    ...
    if (mode === 'grid') {
        ...
        // 切换至瀑布流模式（JS Masonry 或 CSS Columns 回退）
        const useColumns = isColumnsFallbackEnabled();

        // 移除其它布局类
        safeClassList(grid, 'remove', 'grid-mode');
        safeClassList(grid, 'remove', 'masonry-mode');
        safeClassList(grid, 'remove', 'columns-mode');

        // 统一清理子元素的内联定位样式，避免从绝对定位残留导致的布局跳动
        Array.from(grid.children).forEach(item => {
            item.style.removeProperty('transform');
            item.style.removeProperty('width');
            item.style.removeProperty('height');
        ...
        if (useColumns) {
            // 使用 CSS Columns 回退方案
            safeClassList(grid, 'add', 'columns-mode');
            // Columns 布局由 CSS 处理，无需JS布局，但仍需保证容器高度自动
            triggerMasonryUpdate();
        } else {
            // 使用 JS 瀑布流布局
            safeClassList(grid, 'add', 'masonry-mode');
            // 初始化/确保容器 ResizeObserver 已就位
            ensureMasonryResizeObserver();
            // 立即执行瀑布流布局与刷新（不延迟，避免闪烁）
            applyMasonryLayout();
            triggerMasonryUpdate();
        }
    }
