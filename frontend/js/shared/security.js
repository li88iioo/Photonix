/**
 * @file frontend/js/shared/security.js
 * @description 定义前端通用的 XSS 防护策略与安全插值工具
 */

import { isDevelopment, SECURITY } from '../core/constants.js';
...
export function sanitizeHtml(html, level = SecurityLevel.BASIC) {
    ...
    // 性能优化：如果输入不包含HTML标签，直接返回（纯文本无需清理）
    if (!/<[^>]+>/i.test(html)) {
        return html;
    }
    ...
    try {
        ...
        // 清理所有顶级节点，使用新容器收集清理后的节点
        const cleanContainer = document.createElement('body');
        ...
        const result = cleanContainer.innerHTML;
        return result || '';
